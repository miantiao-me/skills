...
**人像光照边界**：面部明暗变化本身不等于身份或几何变形；人工复核应把“人物、五官、头发和轮廓是否被重绘或位移”与“光照是否越过允许区域”分开判断。但当 brief 明确将脸部列为禁区时，跨脸光照仍是安全载体范围违规，即使审美上被认为好看，也不能事后放宽 brief 或自动允许扫脸。
...
- **accepted**：技术通过，并完成可靠视觉复核，所有硬约束与预期运动成立；不等于封面授权。用户审美满意度单独记录，不能替代这些条件。
- **rejected**：身份/文字/关键几何改变、镜头移动、硬切或闪烁、新对象、首帧错误，或主运动越过 brief 指定的禁区/保护边界，直接拒绝。
- **retryable**：仅光强、节奏或辅助竞争问题，可移除辅助、减幅后重新 plan；不包括越过 brief 禁区的光照范围违规；再次付费仍需确认。
- **needs_review**：无法可靠判断，保持待复核，不能推定通过。

### 人工复核报告的分开记录

以下是人工报告的组织方式，不是 CLI Brief 或 Review JSON 的新增字段：

| 维度 | 记录重点 |
| --- | --- |
| 审美满意度 | 用户或评审是否喜欢；仅作独立反馈，不能抹除硬约束失败或改写 `accepted`。 |
| brief 遵从 | 载体、主运动、幅度和禁止项是否按 brief 执行；越过明确禁区即不遵从。 |
| 身份、几何与文字 | 分开判断人物身份、五官/头发/轮廓、构图几何和文字是否重绘、位移或变形；面部光照变化本身不自动等同身份或几何变形。 |
| 光照覆盖 | 判断光效是否留在 brief 指定的安全区域；若脸部是禁区，跨脸光照即使没有重绘五官也属于范围违规。 |
| 循环连续性 | 记录是否回近初始、是否有切换或闪烁；接触表和抽样只能提供提示，不能证明全程无缝，也不能替代完整播放。 |
| 技术结果 | 独立记录视频流、音轨、尺寸、时长等自动检查；技术失败不能被审美满意度覆盖。 |

最终结论仍按现有四态和硬约束门禁给出。查看 contact sheet 只是初筛，须播放全片、核对输入首帧与首尾。SSIM 不能单独证明无缝。CLI 的技术状态只有 `needs_review` / `fail`，`accepted` 恒为 false；上面四态是 agent 的视觉验收结论，不是 CLI 自动分类。
